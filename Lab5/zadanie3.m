function [matrix_condition_numbers, max_coefficients_difference_1, max_coefficients_difference_2] = zadanie3()
    ...
    for i = 1:length(N)
        ...
        % Wyznaczenie współczynników wielomianu interpolującego
    ...
    % Chart 1: Matrix condition numbers
    subplot(3, 1, 1);
    semilogy(N, matrix_condition_numbers, '-o', 'LineWidth', 1.5);
    ...
    % Chart 2: Max coefficients difference for linear function
    subplot(3, 1, 2);
    plot(N, max_coefficients_difference_1, '-o', 'LineWidth', 1.5);
    title('Maksymalna różnica współczynników dla funkcji liniowej');
    xlabel('Rozmiar macierzy Vandermonde');
    ylabel('Maksymalna różnica współczynników');
    grid on;

    % Chart 3: Max coefficients difference for perturbed linear function
    subplot(3, 1, 3);
    plot(N, max_coefficients_difference_2, '-o', 'LineWidth', 1.5);
    title('Maksymalna różnica współczynników dla zaburzonej funkcji liniowej');
    xlabel('Rozmiar macierzy Vandermonde');
    ylabel('Maksymalna różnica współczynników');
    grid on;
